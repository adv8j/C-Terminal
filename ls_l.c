#include "headers.h"
int ls_l_actual_implementation(char *filename);
...
int ls_l(char *cwd)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        continue;
        ls_l_actual_implementation(namelist[i]->d_name);
        free(namelist[i]);
    }
    free(namelist);
    return 0;
...
int ls_l_actual_implementation(char *filename)
{
    ...
    struct passwd *owner_name = getpwuid(my_stat.st_uid);
    printf("%s ", owner_name->pw_name);
